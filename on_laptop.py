...
#sys.path.insert(1, os.path.abspath(os.path.join(src_dir, 'lib')))
...
class listener(Leap.Listener):
	sample = 30						#specify the number of sample should be captured
	enable = 0							#this flag for starting the sample capturing
	main_enable = 0						#this flag enabled when sample capturing is finished
...
#example: count = word.is_this(data,2)
	def is_this(self,d,n):								#chack inputed gesture is this or not
		...
		if (n == self.no_of_hand):
			...
			for i in xrange(30):								#comarring each column of one matrix with all other column od another metrix, this way i'm comparing every two column between two metrix
			...
			print(self.word,' (',cnt,') ',cnt*100/(30*30),'%')  #show the result
			#print time.time()-a
			return cnt	
